...
| ------------ | ----- | ------ | ----- | ----- | 
| 市场行情       | [GET /market/history/kline](#get-markethistorykline-%E8%8E%B7%E5%8F%96k%E7%BA%BF%E6%95%B0%E6%8D%AE)  | GET | K线  | N |
| 市场行情       | [GET /market/detail/merged](#get-marketdetailmerged-%E8%8E%B7%E5%8F%96%E8%81%9A%E5%90%88%E8%A1%8C%E6%83%85ticker)  | GET | 滚动24小时交易和最优报价聚合行情(单个symbol)  | N |
| 市场行情       | [GET /market/tickers](#get-markettickers)  | GET | 全部symbol的交易行情 | N |
| 市场行情       | [GET /market/depth](#get-marketdepth-获取-market-depth-数据)  | GET | 市场深度行情（单个symbol） | N |
| 市场行情       | [GET /market/trade](#get-markettrade-获取-trade-detail-数据)  | GET | 单个symbol最新成交记录 | N |
| 市场行情       | [GET /market/history/trade](#get-markethistorytrade-批量获取最近的交易记录)  | GET | 单个symbol批量成交记录 | N |
| 市场行情       | [GET /market/detail](#get-marketdetail-获取-market-detail-24小时成交量数据)  | GET | 滚动24小时交易聚合行情(单个symbol)  |N |
| 交易品种信息       | [GET /v1/common/symbols](#get-v1commonsymbols-查询pro站支持的所有交易对及精度)  | GET | 交易品种的计价货币和报价精度  |N |
| 交易品种信息       | [GET /v1/common/currencys](#get-v1commoncurrencys-查询pro站支持的所有币种)  | GET | 交易币种列表  |N |
| 系统信息       | [GET /v1/common/timestamp](#get-v1commontimestamp-查询系统当前时间)  | GET | 查询当前系统时间  |N |
|账户信息	|[GET /v1/account/accounts](#get-v1accountaccounts)|	GET| 查询用户的所有账户状态| Y|
|账户信息	|[GET /v1/account/accounts/{account-id}/balance](#get-v1accountaccountsaccount-idbalance-查询pro站指定账户的余额)|GET|	查询指定账户余额	|Y|
|交易	|[POST/v1/order/orders/place](#post-v1orderordersplace-pro站下单)|POST|	下单	|Y|
|交易	|[POST/v1/order/orders/{order-id}/submitcancel](#post-v1orderordersorder-idsubmitcancel--申请撤销一个订单请求)|POST|	按order-id撤销一个订单	|Y|
|交易	|[POST /v1/order/orders/batchcancel](#post-v1orderordersbatchcancel-批量撤销订单)|POST|	按order_id, 批量撤销订单（up to 50)	|Y|
|交易	|[POST /v1/order/batchCancelOpenOrders](#post--v1orderbatchcancelopenorders--批量取消符合条件的订单)|POST|	按订单条件批量撤销订单（up to 100)	|Y|
|用户订单信息	|[GET /v1/order/orders/{order-id}](#get-v1orderordersorder-id-查询某个订单详情)|GET|根据order-id查询订单详情|Y|
|用户订单信息	|[GET /v1/order/orders/{order-id}/matchresults](#get-v1orderordersorder-idmatchresults--查询某个订单的成交明细)	|GET| 根据order-id查询订单的成交明细	|Y|
|用户订单信息	|[GET /v1/order/orders](#get-v1orderorders-查询当前委托历史委托)	|GET|查询用户当前委托、或历史委托订单 (up to 100)	|Y|
|用户订单信息	|[GET /v1/order/matchresults](#get-v1ordermatchresults-查询当前成交历史成交)	|GET|查询用户当前成交、历史成交|Y|
|用户订单信息	|[GET /v1/order/openOrders](#get-v1orderopenorders-获取所有当前帐号下未成交订单)	|GET|查询用户当前未成交订单 (up to 500)|Y|	
|充提币	|[POST /v1/dw/withdraw/api/create](#post-v1dwwithdrawapicreate-申请提现虚拟币)	|POST|申请提币|	Y|
|充提币	|[POST /v1/dw/withdraw-virtual/{withdraw-id}/cancel](#post-v1dwwithdraw-virtualwithdraw-idcancel-申请取消提现虚拟币)|POST| 	撤销提币申请|Y|
|充提币	|[GET /v1/query/deposit-withdraw](#get-v1querydeposit-withdraw-查询虚拟币充提记录)	|GET|查询充提记录|Y|
|杠杆交易	|[POST /v1/dw/transfer-in/margin](#post-v1dwtransfer-inmargin--现货账户划入至借贷账户)|POST|从币币交易账户划转至杠杆账户|Y|
|杠杆交易	|[POST /v1/dw/transfer-out/margin](#post-v1dwtransfer-outmargin--借贷账户划出至现货账户)|POST|从杠杆账户划转至币币交易账户|Y|
|杠杆交易	|[POST /v1/margin/orders](#post-v1marginorders-申请借贷)	|POST|申请借贷|Y|
|杠杆交易	|[POST /v1/margin/orders/{order-id}/repay](#post-v1marginordersorder-idrepay-归还借贷)	|POST|归还借贷|Y|
|杠杆交易	|[GET /v1/margin/loan-orders](#get-v1marginloan-orders--借贷订单)	|GET|查询借贷记录|Y|
|杠杆交易	|[GET /v1/margin/accounts/balance](#get-v1marginaccountsbalance-借贷账户详情)	|GET|查询杠杆账户余额|Y|
|ETF换入换出	|[GET /etf/swap/config](#get-etfswapconfig)|GET|ETF换入换出的基本信息，ETF换入换出状态，以及ETF的成分结构。|Y|
|ETF换入换出	|[POST/etf/swap/in](#post-etfswapin)|POST|用户可以通过该接口换入一定数量的ETF.|Y|
|ETF换入换出	|[POST/etf/swap/out](#post-etfswapout)|POST|用户可以通过该接口换出一定数量的ETF.|Y|
|ETF换入换出	|[GET/etf/list](#get-etflist)|GET|ETF换入换出操作的明细记录。最多返回 100 条记录。|Y|
|ETF换入换出	|[GET/quotation/market/history/kline](#get-quotationmarkethistorykline-获取etf净值)|GET|ETF净值的K线|N|
...
####  POST  /v1/order/batchCancelOpenOrders  批量取消符合条件的订单
...
| symbol     | true  | string | 交易对   | btcusdt, bchbtc, rcneth ... |    |
| types      | false | string | 查询的订单类型组合，使用','分割   |      | buy-market：市价买, sell-market：市价卖, buy-limit：限价买, sell-limit：限价卖, buy-ioc：IOC买单, sell-ioc：IOC卖单 |
| start-date | false | string | 查询开始日期, 日期格式yyyy-mm-dd | -61 days     | [-61day, end-date] |
